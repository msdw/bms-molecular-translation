...
class Net(nn.Module):
    ...
    def __init__(self,):
        ...
        self.text_decode = TransformerDecode(patch_dim, text_dim, ff_dim, num_head, num_layer)

        #---
        self.logit  = nn.Linear(text_dim, vocab_size)
    ...
    def forward_argmax_decode(self, patch, coord, patch_pad_mask):
        ...
        patch_dim = configure.patch_dim
        text_dim  = configure.text_dim
        num_layer = configure.num_layer
        num_head  = configure.num_head
        ff_dim    = configure.ff_dim
        STOI = {
            '<sos>': 190,
            '<eos>': 191,
            '<pad>': 192,
...
def run_check_net():
    ...
    predict = net.forward_argmax_decode(patch, coord, patch_pad_mask)
    print(predict.shape)
